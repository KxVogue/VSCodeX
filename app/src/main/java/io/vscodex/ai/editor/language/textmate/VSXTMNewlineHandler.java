...
public class VSXTMNewlineHandler implements NewlineHandler {
  private final VSXTMLanguage language;
  ...
  public VSXTMNewlineHandler(VSXTMLanguage language) {
  ...
  protected Pair<String, String> getIndentForEnter(Content text, CharPosition position) {
    ...
    var afterEnterAction = getInheritIndentForLine(new VSXTMNewlineHandler.WrapperContentImp(text, position.line, beforeEnterText), true, position.line + 1);
   ...
   */
  @Nullable
  private VSXTMNewlineHandler.InheritIndentResult getInheritIndentForLine(VSXTMNewlineHandler.WrapperContent wrapperContent,
    ...
    if (line <= 0) {
      return new VSXTMNewlineHandler.InheritIndentResult("", null);
    }
    ...
    if (indentRulesSupport.shouldIncrease(precedingUnIgnoredLineContent) || indentRulesSupport.shouldIndentNextLine(precedingUnIgnoredLineContent)) {
      return new VSXTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(precedingUnIgnoredLineContent, 0, precedingUnIgnoredLineContent.length()), EnterAction.IndentAction.Indent, precedingUnIgnoredLine);
    } else if (indentRulesSupport.shouldDecrease(precedingUnIgnoredLineContent)) {
      return new VSXTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(precedingUnIgnoredLineContent, 0, precedingUnIgnoredLineContent.length()), null, precedingUnIgnoredLine);
    } else {
      ...
      if (precedingUnIgnoredLine == 0) {
        return new VSXTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(wrapperContent.getLineContent(precedingUnIgnoredLine)), null, precedingUnIgnoredLine);
      }
      ...
      if (((previousLineIndentMetadata & (IndentRulesSupport.IndentConsts.INCREASE_MASK | IndentRulesSupport.IndentConsts.DECREASE_MASK)) == 0) && (previousLineIndentMetadata & IndentRulesSupport.IndentConsts.INDENT_NEXTLINE_MASK) == 0 && previousLineIndentMetadata > 0) {
        ...
        return new VSXTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(wrapperContent.getLineContent(stopLine + 1)), null, stopLine + 1);
      }

      if (honorIntentialIndent) {
        return new VSXTMNewlineHandler.InheritIndentResult(
      ...
      for (var i = precedingUnIgnoredLine; i > 0; i--) {
        ...
        if (indentRulesSupport.shouldIncrease(lineContent)) {
          return new VSXTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(lineContent), EnterAction.IndentAction.Indent, i);
          ...
          return new VSXTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(lineContent), null, i);
        }
      }

      return new VSXTMNewlineHandler.InheritIndentResult(TextUtils.getLeadingWhitespace(wrapperContent.getLineContent(1)), null, 1);
    }
  }
   ...
   */
  public int getPrecedingValidLine(VSXTMNewlineHandler.WrapperContent content, int lineNumber) {
  ...
  /*
     ...
     */
  private static class WrapperContentImp implements VSXTMNewlineHandler.WrapperContent {
