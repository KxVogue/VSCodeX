...
package io.vscodex.ai.ui.screens.editor.components
...
import io.vscodex.ai.activities.MarkdownPreviewActivity
import io.vscodex.ai.activities.TerminalActivity
import io.vscodex.ai.app.strings
import io.vscodex.ai.compose.LocalMenuManager
import io.vscodex.ai.core.EventManager
import io.vscodex.ai.core.components.Tooltip
...
import io.vscodex.ai.editor.events.OnContentChangeEvent
import io.vscodex.ai.editor.events.OnKeyBindingEvent
import io.vscodex.ai.extensions.open
import io.vscodex.ai.file.extension
import io.vscodex.ai.file.wrapFile
import io.vscodex.ai.keyboard.model.Command.Companion.newCommand
import io.vscodex.ai.preferences.pythonDownloaded
...
import io.vscodex.ai.utils.isFileRunnable
import io.vscodex.ai.utils.launchWithProgressDialog
import io.vscodex.ai.webserver.LocalHttpServer
import io.vscodex.ai.plugins.editor.Position
import io.vscodex.ai.plugins.event.CursorChangedEvent
import io.vscodex.ai.plugins.event.TextChangeEvent
...
fun FileMenu(
    ...
    // In-app file picker – avoids Android SAF "Open with" dialog entirely
    ...
    // NOT wrapped in remember – needs to capture the current showFilePicker setter
    val launchOpenFile = { showFilePicker = true }

    // Show in-app file picker dialog when triggered
    if (showFilePicker) {
        io.vscodex.ai.ui.components.FilePickerDialog(
            onFilePicked = { file ->
                showFilePicker = false
                editorViewModel.addFile(file.wrapFile())
            },
    ...
    // Keep a stable ref to launchOpenFile so LaunchedEffect(Unit) always calls the latest version
    val launchOpenFileRef = remember { mutableStateOf(launchOpenFile) }
    launchOpenFileRef.value = launchOpenFile

    LaunchedEffect(Unit) {
        commandPaletteManager.addCommand(
            newCommand("New File", "Ctrl+N") { launchFileCreate() },
            newCommand("Open File", "Ctrl+O") { launchOpenFileRef.value() },
