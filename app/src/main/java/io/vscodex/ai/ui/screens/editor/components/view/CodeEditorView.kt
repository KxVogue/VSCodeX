package io.vscodex.ai.ui.screens.editor.components.view
...
import io.vscodex.ai.editor.databinding.LayoutCodeEditorBinding
import io.vscodex.ai.editor.language.textmate.VSXTMLanguage
import io.vscodex.ai.events.OnPreferenceChangeEvent
import io.vscodex.ai.file.File
import io.vscodex.ai.file.extension
import io.vscodex.ai.preferences.PREF_APPEARANCE_UI_MODE_KEY
import io.vscodex.ai.preferences.PREF_EDITOR_COLORSCHEME_KEY
import io.vscodex.ai.preferences.PREF_EDITOR_DELETELINEONBACKSPACE_KEY
import io.vscodex.ai.preferences.PREF_EDITOR_DELETETABONBACKSPACE_KEY
import io.vscodex.ai.preferences.PREF_EDITOR_FONTLIGATURES_KEY
import io.vscodex.ai.preferences.PREF_EDITOR_FONT_KEY
import io.vscodex.ai.preferences.PREF_EDITOR_FONT_SIZE_KEY
import io.vscodex.ai.preferences.PREF_EDITOR_INDENT_KEY
import io.vscodex.ai.preferences.PREF_EDITOR_LINENUMBER_KEY
import io.vscodex.ai.preferences.PREF_EDITOR_STICKYSCROLL_KEY
import io.vscodex.ai.preferences.PREF_EDITOR_USETAB_KEY
import io.vscodex.ai.preferences.PREF_EDITOR_WORDWRAP_KEY
import io.vscodex.ai.preferences.editorColorScheme
import io.vscodex.ai.preferences.editorDeleteLineOnBackspace
import io.vscodex.ai.preferences.editorDeleteTabOnBackspace
import io.vscodex.ai.preferences.editorFont
import io.vscodex.ai.preferences.editorFontLigatures
import io.vscodex.ai.preferences.editorFontSize
import io.vscodex.ai.preferences.editorIndent
import io.vscodex.ai.preferences.editorLineNumber
import io.vscodex.ai.preferences.editorStickyScroll
import io.vscodex.ai.preferences.editorUseTab
import io.vscodex.ai.preferences.editorWordWrap
import io.vscodex.ai.providers.GrammarProvider
import io.vscodex.ai.resources.R
import io.vscodex.ai.utils.cancelIfActive
...
class CodeEditorView(context: Context, file: File) : LinearLayout(context) {
    ...
    private fun updateEditorIndent() {
        (editor.editorLanguage as? VSXTMLanguage)?.tabSize = editorIndent
        editor.tabWidth = editorIndent
    }

    private fun updateEditorUseTab() {
        (editor.editorLanguage as? VSXTMLanguage)?.useTab(editorUseTab)
    }
    ...
    private suspend fun createLanguage(): Language {
        ...
        return if (scopeName != null) {
            VSXTMLanguage.create(scopeName, GrammarRegistry.getInstance(), true).apply {
