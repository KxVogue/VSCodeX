...
import io.vscodex.ai.activities.CrashActivity
import io.vscodex.ai.core.EventManager
import io.vscodex.ai.extensions.doIfNull
...
class VSXApplication : BaseApplication() {
    ...
    companion object {
        ...
        private const val TAG = "VSXApplication"

        private var appInstance: VSXApplication? = null

        init {
            System.loadLibrary("vsx")
        }

        @JvmStatic
        @Synchronized
        fun getInstance(): VSXApplication {
            doIfNull(appInstance) {
                appInstance = VSXApplication()
            }
