...
package io.vscodex.ai.core.settings
...
object Settings {
    ...
    object General {
        ...
        @Composable
        fun rememberDesktopMode() = rememberPreference(
            key = DESKTOP_MODE,
            defaultValue = false
        )
    }
    ...
    object AI {
        ...
        val AI_REWRITE_ENABLED  = booleanPreferencesKey("ai_rewrite_enabled")
        val OPENROUTER_MODEL   = stringPreferencesKey("openrouter_model")

        @Composable
        fun rememberOpenRouterApiKey() =
            rememberPreference(key = OPENROUTER_API_KEY, defaultValue = "")

        @Composable
        fun rememberOpenRouterModel() =
            rememberPreference(
        ...
        @Composable
        fun rememberAiRewriteEnabled() =
            rememberPreference(key = AI_REWRITE_ENABLED, defaultValue = false)
    }
    ...
    object Terminal {
        ...
        /** Matches the choices in TerminalFontPreference. */
        const val FONT_JETBRAINS = "JetBrainsMono-Regular"
        const val FONT_FIRA_CODE = "FiraCode-Regular"

        @Composable
        fun rememberFontSize() = rememberPreference(key = FONT_SIZE, defaultValue = 23f)

        @Composable
        fun rememberFontFamily() = rememberPreference(key = FONT_FAMILY, defaultValue = FONT_JETBRAINS)
    }
