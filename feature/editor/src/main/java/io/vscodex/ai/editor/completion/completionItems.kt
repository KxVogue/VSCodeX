...
abstract class VSXCompletionItem(
...
) : VSXCompletionItem(completionKind, label, desc) {
...
class AICompletionItem(
    ...
    desc: CharSequence?,
    private val commitText: String,
) : VSXCompletionItem(CompletionItemKind.AI_GENERATED, label, desc) {
    override fun performCompletion(editor: CodeEditor, text: Content, line: Int, column: Int) {
        text.insert(line, column, commitText)
    }
}
...
) : VSXCompletionItem(CompletionItemKind.SNIPPET, label, desc) {
...
/**
 * Completion item kinds — mirrors VS Code's CompletionItemKind.
 *
 * IDENTIFIER  → Function / method  (orange-red  badge "F")
 * KEYWORD     → Language keyword   (purple      badge "K")
 * VALUE       → Variable / const   (blue        badge "V")
 * CLASS       → Type / class name  (teal        badge "C")
 * SNIPPET     → Code snippet       (green       badge "S")
 * AI_GENERATED→ AI suggestion      (teal        badge "A")
 * FILE        → File path          (slate       badge "F")
 * FOLDER      → Directory          (slate       badge "D")
 * TAG         → HTML/XML tag       (orange      badge "T")
 * ATTRIBUTE   → HTML attribute     (red-pink    badge "@")
...
enum class CompletionItemKind {
    ...
    TAG,
    ATTRIBUTE,
    IDENTIFIER,  // used for functions/methods
    UNKNOWN,
}
